...
在“服务管理”页面点击“新建终端”，或在终端页“全部终端”标题右侧点击加号。
...
4. 在终端的更多操作中可以将终端移至其他服务分组，也可以打开终端、停止服务或删除终端。
...
终端页左侧将“终端工作区”和“全部终端”分开显示。工作区是用于摆放分屏的持久化视图，不是服务分组：服务分组用于服务管理、勾选与批量指令；同一个终端可以加入多个终端工作区。

- 点击工作区，会按该组成员顺序显示 1–4 个独立真实 PTY 分屏；工作区可以为空。
- 点击“终端工作区”标题旁的加号可新建空工作区；工作区的三点菜单可改名、勾选成员或删除工作区定义。
- 在“全部终端”中点击终端，会替换为该终端的单屏视图；终端右侧三点菜单可以把它加入尚未包含它、且未满的工作区。
- 每个工作区最多包含 **4 个不同终端**；第 5 个成员不能加入。删除工作区只删除其定义，不会停止服务、删除终端配置或关闭 tmux 会话。
- 点击任意分屏会切换当前终端。侧边指令库会作用于当前聚焦格；每格仍各自保留搜索、滚动、连接状态、复制、粘贴和清屏操作。
- 1 / 2 个成员保持现有横向布局；3 个成员显示横向三屏，可拖动两条竖线调节三列宽度；恰好 4 个成员时显示田字形 2×2 分屏。四屏中间竖线可调整左右列宽，横线可调整上下行高；双击任一分割线恢复均分。
- 四屏比例会按工作区保存在 `servers.json`，不同工作区互不影响。窄窗口会自动改为纵向单列并隐藏分割线；当前选中的工作区视图仍不会在重启后自动恢复。
- 删除终端时，应用会从所有终端工作区自动移除该终端引用；其余成员和分屏保持不受影响。
...
正式配置文件：`backend/servers.json`。配置包含服务分组、终端、`terminalWorkspaces` 终端工作区、命令、编排、定时任务和设置。保存使用临时文件替换，降低中断造成半截 JSON 的风险。日志目录为 `backend/logs`；打包版使用应用目录下的 `data\logs`。
