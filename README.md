...
它适合管理 Minecraft、测试服务、脚本任务和其他需要多个终端协作的本地项目。终端、服务、命令、编排、定时任务和日志都保存在本机，不需要云端账号；后端默认只监听 `127.0.0.1`。
...
把每个服务实际使用的命令写在对应终端的 `startCommand`，再在编排中使用“服务动作”步骤调用。不要依赖固定的 `./mysrv.py start`，因为每个服务的目录和脚本可能不同。
